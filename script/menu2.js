//some issues: i think the image in the cart should be from the database or menu_img not from local storage
//check css hidden/visible
...
cartCheckoutBtn.addEventListener('click', async function () {
  ...
  const response = await fetch('create_order.php', {
    ...
    body: JSON.stringify({}) // Currently, we're not sending any data for creating a new order
  });
  ...
  for (let item of cartItems) {
    await fetch('add_item_order.php', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
